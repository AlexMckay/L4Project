...
public class DirectoryContents {
    ...
    private File pointsPath;
    private int iterator;

    public DirectoryContents(File dirPath) {
        iterator = 0;
        pointsPathString = dirPath.getAbsolutePath() + "\\FaceClicker_Output";
        
        ArrayList<String> allFiles = listFiles(dirPath);
        imageList = parseImages(allFiles);
        
        pointsPath = new File(pointsPathString);
        if(!pointsPath.exists()){
            pointsPath.mkdir();
        }
        ArrayList<String> pointsFiles = listFiles(pointsPath);
        pointList = parsePoints(pointsFiles);
    }
    ...
    public void refreshPointlist() {
        ArrayList<String> files = listFiles(pointsPath);
        pointList = parsePoints(files);
    }
